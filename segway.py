...
def FastReadGyro(infile):
    infile.seek(0)    
    val = infile.read()
    # debug_print(val)
    return(int(val.decode().strip()) - 3013)
...
# Range Sensor (jano)
rangeSensor = ev3.UltrasonicSensor()
...
while True:
    ...
    # Let the user stabilize after depress
    time.sleep(0.5)
    ...
    while not touchSensorPressed: 
        ...
        if 5 < rangeSensor.distance_centimeters < 10:
            speed = 50
        ...
        # debug_print("gyrorate:{:.4f}; gyroestim:{:4f}".format(gyroRate, gyroEstimatedAngle))
        ...
        # eprint("motor: {0}".format(motorDutyCycle))        
